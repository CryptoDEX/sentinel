...
class CryptoDexDaemon():
    def __init__(self, **kwargs):
        ...
        # memoize calls to some cryptodexd methods
    ...
    @classmethod
    def from_cryptodex_conf(self, cryptodex_dot_conf):
        from cryptodex_config import CryptoDexConfig
        config_text = CryptoDexConfig.slurp_config_file(cryptodex_dot_conf)
        creds = CryptoDexConfig.get_rpc_creds(config_text, config.network)
    ...
    def get_my_gobject_votes(self, object_hash):
        ...
        if not self.gobject_votes.get(object_hash):
            ...
            self.gobject_votes[object_hash] = cryptodexlib.parse_raw_votes(raw_votes)
    ...
    def we_are_the_winner(self):
        ...
        winner = cryptodexlib.elect_mn(block_hash=current_block_hash, mnlist=mn_list)
        my_vin = self.get_current_masternode_vin()
    ...
    def estimate_block_time(self, height):
        ...
        future_seconds = cryptodexlib.blocks_to_seconds(diff)
    ...
    def has_sentinel_ping(self):
        ...
        return (getinfo['protocolversion'] >= config.min_cryptodexd_proto_version_with_sentinel_ping)
