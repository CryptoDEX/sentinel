...
class GovernanceClass(object):
    ...
    def vote(self, cryptodexd, signal, outcome):
        return self.go.vote(cryptodexd, signal, outcome)

    # pass thru to GovernanceObject#voted_on
    def voted_on(self, **kwargs):
        return self.go.voted_on(**kwargs)

    def vote_validity(self, cryptodexd):
        if self.is_valid():
            printdbg("Voting valid! %s: %d" % (self.__class__.__name__, self.id))
            self.vote(cryptodexd, models.VoteSignals.valid, models.VoteOutcomes.yes)
        else:
            printdbg("Voting INVALID! %s: %d" % (self.__class__.__name__, self.id))
            self.vote(cryptodexd, models.VoteSignals.valid, models.VoteOutcomes.no)
    ...
    def list(self):
        ...
        # return a dict similar to cryptodexd "gobject list" output
    ...
    def cryptodexd_serialise(self):
        import cryptodexlib
        return cryptodexlib.SHIM_serialise_for_cryptodexd(self.serialise())
