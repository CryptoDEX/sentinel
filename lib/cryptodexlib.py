...
def is_valid_cryptodex_address(address, network='mainnet'):
    ...
    cryptodex_version = 140 if network == 'testnet' else 23
    ...
    try:
        ...
        # rescue from exception, not a valid CryptoDex address
        return False

    if (address_version != cryptodex_version):
        return False
...
# shims 'til we can fix the cryptodexd side
def SHIM_serialise_for_cryptodexd(sentinel_hex):
    from models import CXDD_GOVOBJ_TYPES
    # unpack
    ...
    # add 'type' attribute
    obj[1]['type'] = CXDD_GOVOBJ_TYPES[govtype]

    # superblock => "trigger" in cryptodexd
    if govtype == 'superblock':
        obj[0] = 'trigger'

    # cryptodexd expects an array (even though there is only a 1:1 relationship between govobj->class)
    obj = [obj]

    # re-pack
    cryptodexd_hex = serialise(obj)
    return cryptodexd_hex


# shims 'til we can fix the cryptodexd side
def SHIM_deserialise_from_cryptodexd(cryptodexd_hex):
    from models import CXDD_GOVOBJ_TYPES

    # unpack
    obj = deserialise(cryptodexd_hex)

    # shim from cryptodexd
...
def did_we_vote(output):
    ...
    try:
        detail = output.get('detail').get('cryptodex.conf')
