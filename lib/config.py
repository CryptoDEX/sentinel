...
sentinel_cfg = CryptoDexConfig.tokenize(sentinel_config_file)
sentinel_version = "1.1.0"
min_cryptodexd_proto_version_with_sentinel_ping = 70207


def get_cryptodex_conf():
    home = os.environ.get('HOME')

    cryptodex_conf = os.path.join(home, ".cryptodexcore/cryptodex.conf")
    if sys.platform == 'darwin':
        cryptodex_conf = os.path.join(home, "Library/Application Support/CryptoDexCore/cryptodex.conf")

    cryptodex_conf = sentinel_cfg.get('cryptodex_conf', cryptodex_conf)

    return cryptodex_conf
...
cryptodex_conf = get_cryptodex_conf()
network = get_network()
db = get_db_conn()
