...
def has_cryptodex_conf():
    import config
    import io

    valid_cryptodex_conf = False

    # ensure cryptodex_conf exists & readable
    #
    # if not, print a message stating that CryptoDex Core must be installed and
    # configured, including JSONRPC access in cryptodex.conf
    try:
        f = io.open(config.cryptodex_conf)
        valid_cryptodex_conf = True
    except IOError as e:
        print(e)

    return valid_cryptodex_conf
...
def main():
    ...
    if not has_cryptodex_conf():
        print("CryptoDexCore must be installed and configured, including JSONRPC access in cryptodex.conf")
        sys.exit(1)
