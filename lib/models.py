...
class GovernanceObject(BaseModel):
    ...
    # sync cryptodexd gobject list with our local relational DB backend
    @classmethod
    def sync(self, cryptodexd):
        golist = cryptodexd.rpc_command('gobject', 'list')
        ...
        try:
            ...
            for item in golist.values():
                (go, subobj) = self.import_gobject_from_cryptodexd(cryptodexd, item)
    ...
    @classmethod
    def import_gobject_from_cryptodexd(self, cryptodexd, rec):
        import decimal
        import cryptodexlib
        import inflection
        ...
        # shim/cryptodexd conversion
        object_hex = cryptodexlib.SHIM_deserialise_from_cryptodexd(object_hex)
        objects = cryptodexlib.deserialise(object_hex)
        subobj = None
        ...
        # exclude any invalid model data from cryptodexd...
        valid_keys = subclass.serialisable_fields()
        subdikt = {k: dikt[k] for k in valid_keys if k in dikt}

        # get/create, then sync vote counts from cryptodexd, with every run
        ...
        # get/create, then sync payment amounts, etc. from cryptodexd - CryptoDexd is the master
        try:
            newdikt = subdikt.copy()
            newdikt['object_hash'] = object_hash
            if subclass(**newdikt).is_valid() is False:
                govobj.vote_delete(cryptodexd)
                return (govobj, None)

            subobj, created = subclass.get_or_create(object_hash=object_hash, defaults=subdikt)
        except Exception as e:
            # in this case, vote as delete, and log the vote in the DB
            printdbg("Got invalid object from cryptodexd! %s" % e)
            govobj.vote_delete(cryptodexd)
            return (govobj, None)
    ...
    def vote_delete(self, cryptodexd):
        if not self.voted_on(signal=VoteSignals.delete, outcome=VoteOutcomes.yes):
            self.vote(cryptodexd, VoteSignals.delete, VoteOutcomes.yes)
        return
    ...
    def vote(self, cryptodexd, signal, outcome):
        import cryptodexlib
        ...
        output = cryptodexd.rpc_command(*vote_command)

        # extract vote output parsing to external lib
        voted = cryptodexlib.did_we_vote(output)
        ...
        if voted:
            ...
            self.sync_network_vote(cryptodexd, signal)

    def sync_network_vote(self, cryptodexd, signal):
        printdbg('\tsyncing network vote for object %s with signal %s' % (self.object_hash, signal.name))
        vote_info = cryptodexd.get_my_gobject_votes(self.object_hash)
...
class Proposal(GovernanceClass, BaseModel):
    ...
    govobj_type = CXDD_GOVOBJ_TYPES['proposal']
    ...
    def is_valid(self):
        ...
        try:
            ...
            # payment address is valid base58 cryptodex addr, non-multisig
            if not cryptodexlib.is_valid_cryptodex_address(self.payment_address, config.network):
                printdbg("\tPayment address [%s] not a valid CryptoDex address for network [%s], returning False" % (self.payment_address, config.network))
                return False
    ...
    def is_expired(self, superblockcycle=None):
        ...
        expiration_window_seconds = int(
            (cryptodexlib.blocks_to_seconds(superblockcycle) / 2) +
    ...
    def is_deletable(self):
        ...
        # TBD (item moved to external storage/CryptoDexDrive, etc.)
        return False
    ...
    def get_prepare_command(self):
        import cryptodexlib
        obj_data = cryptodexlib.SHIM_serialise_for_cryptodexd(self.serialise())
    ...
    def prepare(self, cryptodexd):
        try:
            object_hash = cryptodexd.rpc_command(*self.get_prepare_command())
...
class Superblock(BaseModel, GovernanceClass):
    ...
    govobj_type = CXDD_GOVOBJ_TYPES['superblock']
    ...
    def is_valid(self):
        ...
        for addr in addresses:
            if not cryptodexlib.is_valid_cryptodex_address(addr, config.network):
    ...
    def hash(self):
        import cryptodexlib
        return cryptodexlib.hashit(self.serialise())
...
class Watchdog(BaseModel, GovernanceClass):
    ...
    govobj_type = CXDD_GOVOBJ_TYPES['watchdog']
    only_masternode_can_submit = True

    @classmethod
    def active(self, cryptodexd):
        now = int(time.time())
        resultset = self.select().where(
            self.created_at >= (now - cryptodexd.SENTINEL_WATCHDOG_MAX_SECONDS)
        )
        return resultset

    @classmethod
    def expired(self, cryptodexd):
        now = int(time.time())
        resultset = self.select().where(
            self.created_at < (now - cryptodexd.SENTINEL_WATCHDOG_MAX_SECONDS)
        )
    ...
    def is_expired(self, cryptodexd):
        now = int(time.time())
        return (self.created_at < (now - cryptodexd.SENTINEL_WATCHDOG_MAX_SECONDS))

    def is_valid(self, cryptodexd):
        if self.is_expired(cryptodexd):
            return False

        return True

    def is_deletable(self, cryptodexd):
        if self.is_expired(cryptodexd):
            return True
